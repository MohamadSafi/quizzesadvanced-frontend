...
const MultipleChoiceAnswer = ({ quiz, question, setQuestion, answer, index, customGradingSchema }) => {
  ...
  const updateCorrect = (value) => {
    ...
    json.meta.answers[index].text = text;
    json.meta.answers[index].correct = value;
  ...
  return (
    <>
      <div className="w-[964px] flex flex-row py-2.5 px-0 box-border items-center justify-start gap-[15px]">
        {!customGradingSchema &&
        <Checkbox initialState={answer.correct} stateHandler={updateCorrect} enabled={!quiz.archived}/>
        }
        <div className="flex-1 flex flex-row items-start justify-start gap-[50px]">
          ...
          {customGradingSchema && <ScoreDropdown initialState={answer.mark} stateHandler={updateMark} enabled={!quiz.archived}/>}
        </div>
        <div className="flex flex-row pt-0 px-0 pb-1.5 items-start justify-start cursor-pointer" onClick={() => deleteAnswer(index)}>
          <img className="relative w-7 h-7" alt="" src="/cross3.svg" />
        </div>
      </div>
    </>
  );
